...
def simple_hoz_vert_line(x1,y1,x2,y2):
    coords = []
    if x1 == x2:
...
def beethams_line_algorithm(x1, y1, x2, y2):
    sim = simple_hoz_vert_line(x1,y1,x2,y2)
    if sim != []:
        return sim
    dx = abs(x2 - x1)
    dy = abs(y2 - y1)
...
def simple_triangle(x1,y1, x2,y2, x3,y3):
    coords = []
    coords.extend(beethams_line_algorithm(x1,y1, x2,y2))
    coords.extend(beethams_line_algorithm(x2,y2, x3,y3))
    coords.extend(beethams_line_algorithm(x3,y3, x1,y1))
    return coords